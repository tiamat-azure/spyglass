...
describe('observeMatchScore navigate URLs (L5-ADV-06)', () => {
  it('does not score button#confirm against a navigate URL hash as ≥80', () => {
    const failing: RefinedStep = {
      index: 0,
      intent: 'Je navigue',
      action: {
        type: 'navigate',
      ...
      verification: {
        type: 'urlMatches',
        expected: 'https://exemple.test/app#confirm',
        strength: 'strong',
        confirmedByUser: true
      },
      sourceEvents: ['evt_000001']
    };
    expect(observeMatchScore({ selector: 'button#confirm' }, failing)).toBeLessThan(80);
    expect(bestCorrelatedObservation(failing, [{ selector: 'button#confirm' }])).toBeUndefined();
  });
});
